...
import com.kotlin.library.util.getString
...
object SpeechUtil {
    ...
    //播放的文字
    private const val defaultText = "富强、明主、文明、和谐、自由、平等、公正、法制、爱国、敬业、诚信、友善。"

    // 引擎类型
    private const val mEngineType = SpeechConstant.TYPE_CLOUD
    ...
    //音调
    private var pitchValue = "50"

    //音量
    private var volumeValue = "50"
    ...
    private const val language = "zh_cn" //识别语言
    private const val resultType = "json" //结果内容数据格式
    private var dictationResults //听写结果
            : String? = null

    /**
     * 初始化监听。
     */
    private val mTtsInitListener = InitListener { code: Int ->
    ...
    fun init(context: Context?) {
        ...
        // 初始化合成对象
        mTts =
            SpeechSynthesizer.createSynthesizer(mContext, mTtsInitListener)
        /***************  语音听写  */
        ...
        mIatDialog = RecognizerDialog(mContext, mInitListener)
     ...
     * 合成回调监听。
     */
    private val mTtsListener: SynthesizerListener =
            ...
            override fun onCompleted(error: SpeechError) {
                ...
                if (error == null) {
                    ...
                    FileUtil.saveFile(
                        memoryFile,
                        mTotalSize,
                        mContext!!.getExternalFilesDir(null).toString() + "/1.pcm"
                    )
                } else {
                    //异常信息
    ...
    private fun writeToFile(data: ByteArray?) {
        ...
        try {
            if (memoryFile == null) {
                ...
                val mFilepath =
                    mContext!!.getExternalFilesDir(null).toString() + "/1.pcm"
                memoryFile = MemoryFile(mFilepath, 1920000)
                memoryFile!!.allowPurging(false)
            }
            memoryFile!!.writeBytes(data, 0, mTotalSize.toInt(), data.size)
    ...
    private fun setParam() {
        ...
        if (mEngineType == SpeechConstant.TYPE_CLOUD) {
            ...
            //获取缓存值
            voicer = "xiaoyan".getString(Constant.VOICE_NAME)!!
            speedValue = "50".getString(Constant.SPEED)!!
            pitchValue = "50".getString(Constant.PITCH)!!
            volumeValue = "50".getString(Constant.VOLUME)!!
        ...
        mTts!!.setParameter(
            ...
            mContext!!.getExternalFilesDir(null).toString() + "/msc/tts.pcm"
        )
    }
     ...
     * 开始语音预报
     */
    fun startVoiceBroadcast(text: String?, textView: TextView?) {
        var text = text
        tvState = textView
        if (mTts == null) {
     ...
     * @param results
     */
    private fun parsingResult(results: RecognizerResult) {
        //获取解析结果
    ...
    fun setDictationParam() {
        ...
        Log.e(
            TAG,
            "last language:" + mIat!!.getParameter(SpeechConstant.LANGUAGE)
        )
     ...
     * 开始听写
     */
    fun startDictation(speechCallback: SpeechCallback?) {
        ...
        //获取字体所在的控件
        val tvLink =
            Objects.requireNonNull(mIatDialog!!.window)!!.decorView.findViewWithTag<TextView>("textlink")
        tvLink.apply {
            text = " "
            paint.flags = Paint.SUBPIXEL_TEXT_FLAG //取消下划线
            isEnabled = false //禁用点击
        }
    }
    ...
    //语音回调
    private var mSpeechCallback: SpeechCallback? = null
    ...
    interface SpeechCallback {
         ...
         * 听写结果
         */
        fun dictationResults(cityName: String?)
    }
}